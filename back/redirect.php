...
if (isset($_GET['t'])) {
    ...
    try {
        ...
        if (!is_null($user_id)) {
            $sql = "INSERT INTO user_redirects_access (user_id ,redirect_id, cid, token) VALUES (:user_id ,:redirect_id, :cid, :token)";
            $stmt = $db->prepare($sql);
            $params = array(':user_id' => $user_id, ':redirect_id' => $redirect_id, ':cid' => $client_id, ':token' => $token);
            $stmt->execute($params);
        }
