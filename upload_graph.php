...
if (isset($_GET['pdf_id'])) {
    ...
    try {
        ...
        $stmt = $db->prepare("SELECT * FROM pdf_versions WHERE cid=? AND pdf_id=? ORDER BY uploaded_at DESC");
        ...
        <main>
            ...
            <div class="d-flex align-items-center">
                ...
                <form action="" method="get">
                    ...
                    <select name="key" id="" class="mx-2">
                        <?php foreach ($pdf_versions_data as $k => $pd): ?>
                            <option value="<?= $k; ?>"><?= $pd['file_name']; ?></option>
                        <?php endforeach; ?>
                    </select>
                    <button class="btn btn-primary">確認</button>
                </form>
                <a href="./upload_history.php?pdf_id=<?= $pdf_id; ?>&pdf_title=<?= $pdf_title; ?>"
            ...
            <table class="w-50 table table-bordered mx-auto">
                ...
                <?php
                if (isset($_GET['key'])) {
                    $row = $pdf_versions_data[$_GET['key']];
                } else {
                    $row = $pdf_versions_data[0];
                }
                ...
                try {
                    ...
                    foreach ($graph2data as $k => $gd) {
                        $graph2label[] = $k;
                        $graph2page[] = $gd['page_num'];
                        $graph2time[] = $gd['duration'];
                    }
